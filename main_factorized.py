...
class MetricAliasCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if candidates:
            # Prefer a reconstruction-named metric, then a deterministic key.
...
def pack_concept_targets(labels, weights):
    return np.concatenate([labels.astype(np.float32), weights.astype(np.float32)], axis=-1)
...
def expand_experiments(config):
    ...
    for experiment in config["experiments"]:
        experiment_seeds = [experiment["training_seed"]] if "training_seed" in experiment else seeds
...
def build_callbacks(run_directory, experiment, config, model):
    monitor = experiment.get("monitor", config.get("monitor", "val_reconstruction_ssim_metric"))
    # Keras does not prefix metrics for a single-output control model.  Concept
    # models are multi-output and therefore use the reconstruction prefix.
    ...
    if experiment.get("semantic_method", "ste") == "gumbel":
        callbacks.append(
            TemperatureAnnealing(
                ...
                epochs=int(experiment.get("temperature_anneal_epochs", config.get("epochs", 60))),
            )
        )
...
def main(config_path):
    ...
    automatic_control_noise = gaussian_noise_for_rate(selected_entropy_bits, concept_dim)
    img_size = tuple(config.get("img_size", [64, 64]))
    active_split_names = ["train", "validation"]
    if bool(config.get("evaluate_official_test", True)):
    ...
    selected_attributes.to_csv(output_root / "selected_attribute_definitions.csv", index=False)
    ...
    for experiment in expand_experiments(config):
        ...
        control_noise = experiment.get(
            "control_noise_std",
            automatic_control_noise,
        )
        if control_noise == "auto":
            control_noise = automatic_control_noise
        model, encoder, decoder = build_factorized_lite_autoencoder(
            ...
            control_noise_std=float(control_noise),
        )
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        run_name = f"{experiment['name']}_Dc{concept_dim}_seed{seed}_{timestamp}"
        run_directory = output_root / run_name
        run_directory.mkdir(parents=True, exist_ok=True)
        (run_directory / "figures" / "group_interventions").mkdir(parents=True, exist_ok=True)

        reconstruction_loss = make_reconstruction_loss(**config.get("reconstruction_loss", {}))
        learning_rate = float(experiment.get("learning_rate", config.get("learning_rate", 1e-3)))
        ...
        if mode in {"concept", "concept_only"}:
            model.compile(
                ...
                loss_weights={
                    "reconstruction": float(experiment.get("reconstruction_weight", 1.0)),
            ...
            train_targets = {
                ...
                "concepts": pack_concept_targets(labels["train"], weights["train"]),
            }
            validation_targets = {
                "reconstruction": arrays["validation"],
                "concepts": pack_concept_targets(labels["validation"], weights["validation"]),
            }
        else:
            model.compile(
                optimizer=optimizer,
        ...
        (run_directory / "model_summary.txt").write_text(model_summary_text(model), encoding="utf-8")
        (run_directory / "encoder_summary.txt").write_text(model_summary_text(encoder), encoding="utf-8")
        (run_directory / "decoder_summary.txt").write_text(model_summary_text(decoder), encoding="utf-8")
        ...
        history = model.fit(
            ...
            batch_size=int(experiment.get("batch_size", config.get("batch_size", 32))),
            callbacks=callbacks,
            verbose=1,
        )
        batch_size = int(experiment.get("batch_size", config.get("batch_size", 32)))
        ...
        best_index = int(np.argmax(monitor_values) if monitor_mode == "max" else np.argmin(monitor_values))
        result = {
            "run_name": run_name,
        ...
        validation_prediction = model.predict(arrays["validation"], batch_size=batch_size, verbose=0)
        if mode in {"concept", "concept_only"}:
            validation_reconstruction = validation_prediction["reconstruction"]
            probabilities = validation_prediction["concepts"]
            encoded = encoder.predict(arrays["validation"], batch_size=batch_size, verbose=0)
            ...
            clean_reconstruction = decoder.predict(clean_input, batch_size=batch_size, verbose=0)
            ...
            if run_reconstruction_analysis:
                soft_input = probabilities if mode == "concept_only" else [residual, probabilities]
                soft_reconstruction = decoder.predict(soft_input, batch_size=batch_size, verbose=0)
                ground_truth = np.where(weights["validation"] > 0, labels["validation"], hard)
                ground_truth_input = ground_truth if mode == "concept_only" else [residual, ground_truth]
                ground_truth_reconstruction = decoder.predict(
                    ground_truth_input, batch_size=batch_size, verbose=0
                )
                semantic_rows = [
                    reconstruction_summary("model_hard", arrays["validation"], clean_reconstruction),
                    reconstruction_summary("soft_concepts", arrays["validation"], soft_reconstruction),
                    reconstruction_summary(
            ...
            atomic_metrics, group_metrics = evaluate_concepts(
                labels["validation"], weights["validation"], probabilities, selected_attributes
            )
            atomic_metrics.to_csv(run_directory / "concept_metrics.csv", index=False)
            group_metrics.to_csv(run_directory / "concept_group_metrics.csv", index=False)
            result["validation_macro_ap"] = float(atomic_metrics.average_precision.mean())
            ...
            if run_reconstruction_analysis:
                ...
                intervention_image_ids = splits["validation"][intervention_indices] + 1
                ...
                intervention_table = evaluate_group_interventions(
                    ...
                    residual=None if residual is None else residual[intervention_indices],
                    hard_concepts=hard[intervention_indices],
                    selected_attributes=selected_attributes,
                    output_directory=run_directory / "figures" / "group_interventions",
            ...
            encoded = encoder.predict(arrays["validation"], batch_size=batch_size, verbose=0)
            clean_control_reconstruction = decoder.predict(
                [encoded["residual"], encoded["control"]],
                batch_size=batch_size,
                verbose=0,
            )
            control_rng = np.random.default_rng(seed)
            noisy_control = encoded["control"] + control_rng.normal(
                0.0, float(control_noise), size=encoded["control"].shape
            ).astype(np.float32)
            validation_reconstruction = decoder.predict(
                [encoded["residual"], noisy_control],
                batch_size=batch_size,
                verbose=0,
            )
            pd.DataFrame(
                [
                    reconstruction_summary(
                        "continuous_clean_upper_bound",
                        arrays["validation"],
                        clean_control_reconstruction,
                    ),
                    reconstruction_summary(
                        "continuous_rate_proxy_noise",
                        arrays["validation"],
                        validation_reconstruction,
                    ),
                ]
            ).to_csv(run_directory / "control_bottleneck_analysis.csv", index=False)
            np.savez_compressed(
                run_directory / "validation_latents.npz",
        ...
        if bool(config.get("evaluate_official_test", True)):
            ...
            if mode == "control":
                ...
                test_noisy_control = test_encoded["control"] + test_rng.normal(
                    0.0, float(control_noise), size=test_encoded["control"].shape
                ).astype(np.float32)
                test_reconstruction = decoder.predict(
                    [test_encoded["residual"], test_noisy_control],
                    batch_size=batch_size,
                    verbose=0,
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", default="configs/factorized_concepts.json")
