...
def build_residual_autoencoder(
    ...
    x = down_block(x, base_channels * 8)      # 8 -> 4
    ...
    encoder = Model(encoder_input, latent, name="residual_encoder")

    decoder_input = layers.Input(shape=(latent_dim,))

    x = layers.Dense(4 * 4 * base_channels * 8)(decoder_input)
    x = layers.Reshape((4, 4, base_channels * 8))(x)

    x = residual_block(x, base_channels * 8)

    x = up_block(x, base_channels * 4)        # 4 -> 8
    ...
    decoder = Model(decoder_input, decoder_output, name="residual_decoder")
